...
# Custom CSS to make sidebar labels bold and prominent
st.markdown("""
    <style>
    [data-testid="stSidebarNav"] {display: none;}
    .st-emotion-cache-167880u {font-weight: bold; font-size: 1.2rem;}
    </style>
...
# 2. Sidebar Navigation - Enhanced Visibility
st.sidebar.markdown("# ⬅️ REQUIRED: START HERE")
...
with tab1:
    ...
    if mic_type == "Headset" or laptop_loc_choice == "Other": 
        # Note: We can assume "Other" often implies complexity needing a tech
    ...
    if final_tech_on_site:
        lines.append("• SPATIAL: Tech Table required for Technician.")

    # Universal Connectivity Truth
    ...
    if laptop_src == "Ours":
        ...
        lines.append(f"• INPUT: Client Laptop ({laptop_loc}). Technician must verify signal path. {power_note}{adapter_note}.")
...
with tab2:
    ...
    st.info("Note: Choosing a headset mandates a Technician and a Tech Table.")
    ...
    * **If No Preference:** "I recommend our tech table. It’s the most elegant look for the room and ensures our technician can manage everything seamlessly behind the scenes."
    """)
    
    st.info("Technician Justification (Internal Only): The tech table provides a central station for the technician to perform active event management—including audio leveling, video switching, and transition timing—without intruding on the presentation space.")
